...
export class JsAnalyzer {
  ...
  private programAst: ESTree.Program | null;
  ...
  /**
   * Releases the parsed AST, source string, and all accumulated maps so that
   * the memory can be reclaimed immediately rather than waiting for the
   * entire owning scope to become unreachable.
   *
   * Call this after `buildScript()` has finished producing its output.
   */
  public dispose(): void {
    ...
    this.dependentsTracker.clear();

    // Clear AST node / metadata references held by extraction states.
    for (const state of this.extractionStates) {
      state.node = undefined;
      state.metadata = undefined;
      state.matchContext = undefined;
